...
def job_submit(script,args):
    ...
    print('#SBATCH -J TransVar',file=out)  
...
#Commandline parsing
disc = "Running TransVarSur"
usage = "usage: %prog [options]"
parser = OptionParser(usage=usage,description = disc)
parser.add_option("-s", action="store", type="string", dest="scenario", help="run TransVarSur in which scenario: use_case or simulation")
...
parser.add_option("-r", action="store", type="string", dest="run_model", help="run the previously trained model (T/F), exclusive to -m and -c")
parser.add_option("-t", action="store", type="string", dest="opt_typ", help="chose optimizer RectifiedAdam or classic Adam")
parser.add_option("-k", action="store", type="string", dest="keep_temp", help="whether to keep temp dir (T/F), default is F",default='False')
